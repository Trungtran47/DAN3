...
import com.example.doan3.databinding.ActivityMonaItemBinding
import com.example.doan3.model.ProductModel

class MonAadapter(private val images: List<ProductModel>) : RecyclerView.Adapter<MonAadapter.ViewHolder>() {

    override fun onCreateViewHolder(parent: ViewGroup, viewType: Int): ViewHolder {
        val binding = ActivityMonaItemBinding.inflate(LayoutInflater.from(parent.context), parent, false)
    ...
    inner class ViewHolder(private val binding: ActivityMonaItemBinding) : RecyclerView.ViewHolder(binding.root) {
        fun bind(feature: ProductModel) {
            binding.nameMonA.text = feature.productName
            binding.priceMonA.text = feature.price
            Glide.with(binding.root)
                .load(feature.image)
                .into(binding.imageMonA)
